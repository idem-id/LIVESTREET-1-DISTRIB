{if $oUserCurrent}
  ...
  <section class="panel panel-default block block-type-activity">
    <div class="panel-body">
      ...
      <div class="block-content">
        <p class="small text-muted">{$aLang.stream_settings_note_filter}</p>
  ...
  <section class="panel panel-default block block-type-activity">
    <div class="panel-body">
      ...
      <div class="block-content">
        <p class="small text-muted">{$aLang.stream_settings_note_follow_user}</p>

        <div class="input-group">
          <input type="text" id="stream_users_complete" autocomplete="off" class="form-control autocomplete-users input-text" />
          <span class="input-group-btn">
            <a title="{$aLang.stream_block_config_append}" href="javascript:ls.stream.appendUser()" class="btn btn-default"><i class="fa fa-user-plus fa-fw"></i></a>
          </span>
        </div>
              ...
              {if !isset($aStreamFriends.$iUserId)}
                <li class="checkbox">
                  <label>
                    ...
                    <a href="{$oUser->getUserWebPath()}" title="{$oUser->getLogin()}"><img src="{$oUser->getProfileAvatarPath(24)}" alt="{$oUser->getLogin()}" class="avatar" /></a>
        ...
        {else}
          ...
          <p id="stream_no_subscribed_users" class="small text-muted">{$aLang.stream_no_subscribed_users}</p>
        {/if}
      </div>
  ...
  <section class="panel panel-default block block-type-activity">
    <div class="panel-body">
      ...
      <div class="block-content">
        <p class="small text-muted">{$aLang.stream_settings_note_follow_friend}</p>
        ...
        {else}
          <p class="small text-muted">{$aLang.stream_no_subscribed_users}</p>
        {/if}
      </div>
