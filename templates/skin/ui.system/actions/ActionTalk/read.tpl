...
<article class="topic topic-type-talk">
  <header class="topic-header">
    ...
    <div class="topic-info">
      <ul class="list-unstyled list-inline small actions">
        ...
        <li class="delete"><a href="{router page='talk'}delete/{$oTalk->getId()}/?security_ls_key={$LIVESTREET_SECURITY_KEY}" title="{$aLang.topic_delete}" onclick="return confirm('{$aLang.talk_inbox_delete_confirm}');" class="actions-delete"><i class="fa fa-trash-o fa-fw"></i></a></li>
      </ul>
    </div>
  </header>
  ...
  <footer class="topic-footer">
    <ul class="text-muted list-unstyled list-inline small topic-info">
      <li class="topic-info-author">
        <a href="{$oUser->getUserWebPath()}"><img src="{$oUser->getProfileAvatarPath(24)}" alt="{$oUser->getLogin()}" class="avatar" /></a>
        <a href="{$oUser->getUserWebPath()}">{$oUser->getLogin()}</a>
      </li>
      <li class="topic-info-favourite"><a href="#" onclick="return ls.favourite.toggle({$oTalk->getId()},this,'talk');" class="favourite {if $oTalk->getIsFavourite()}active{/if}" title="{if $oTalk->getIsFavourite()}{$aLang.topic_del_favourite}{else}{$aLang.topic_add_favourite}{/if}"><span class="glyphicon glyphicon-star-empty"></span></a></li>
