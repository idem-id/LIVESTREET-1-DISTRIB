    ...
    <table class="table table-talk">
      <thead>
        <tr>
          ...
          <th class="small cell-recipients">{$aLang.talk_inbox_target}</th>
          <th class="hidden-xs small cell-favourite"></th>
          <th class="small cell-title">{$aLang.talk_inbox_title}</th>
          <th class="hidden-xs small cell-date">{$aLang.talk_inbox_date}</th>
        </tr>
      </thead>
        ...
        {foreach from=$aTalks item=oTalk}
          ...
          <tr>
            ...
            <td class="hidden-xs cell-favourite">
              <a href="#" onclick="return ls.favourite.toggle({$oTalk->getId()},this,'talk');" class="muted favourite {if $oTalk->getIsFavourite()}active{/if}" title="{if $oTalk->getIsFavourite()}{$aLang.topic_del_favourite}{else}{$aLang.topic_add_favourite}{/if}"><span class="glyphicon glyphicon-star-empty"></span></a>
            </td>
            <td>
              {strip}
